...
# # 初始化Kalman滤波器
KF = KalmanFilter(0.1, 25, 25, 1, 0.1, 0.1)
...
while True:
    ...
    for contour in contours[:5]:
        ...
        # 燈條條件
        is_valid = h/w > 2.4 and h/whole_h > 0.1 and w < 200
        if not is_valid:
            continue
    ...
    for i in range(len(width_array) - 1):
        for j in range(i+1, len(width_array)):
            ...
            b = int((int(Midy_point[i] + height_array[i] / 2) +
                     int(Midy_point[j] + height_array[j] / 2)) / 2)
        ...
        try:
            ...
            cvzone.putTextRect(img, f'Depth:{int(d)}cm', (100, 100), scale=5)
            # 进行位置解算
            ...
            #預測坐標
            (pa, pb) = KF.predict()
            cv2.circle(img, (int(pa), int(pb)), 20, (0, 255, 0), -1)
            #更新坐標
            (ua, ub) = KF.update(centers[0])
            cv2.circle(img, (int(ua), int(ub)), 20, (255, 255, 0), -1)
